;; Found at http://www.emacswiki.org/emacs/Journal
...
(defun journal (filename)
  "Open .md file named after today's date, format YYYY-MM-DD-Day.md,
in subdirectory named in variable journal-dir, set in ~/.emacs,
else in $HOME."
  (interactive
   (progn
     (setq default-directory journal-dir)
     (setq filename (concat (format-time-string "%Y-%m-%d-%a" nil) ".md"))
     (list (read-file-name
